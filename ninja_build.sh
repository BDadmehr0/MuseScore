...
# TLS settings used by CMake's file(DOWNLOAD) for extdeps/soundfont fetches.
# A MinGW/MSYS2 cmake.exe on Windows often does not know where the CA bundle is,
# which shows up as "SSL peer certificate or SSH remote key was not OK (60)".
MUSESCORE_CMAKE_TLS_VERIFY=${CMAKE_TLS_VERIFY:-"ON"}
MUSESCORE_CMAKE_TLS_CAINFO=${CMAKE_TLS_CAINFO:-}
...
CMAKE_TLS_ARGS=(-DCMAKE_TLS_VERIFY="${MUSESCORE_CMAKE_TLS_VERIFY}")
if [ -n "${MUSESCORE_CMAKE_TLS_CAINFO}" ]; then
    CMAKE_TLS_ARGS+=(-DCMAKE_TLS_CAINFO="${MUSESCORE_CMAKE_TLS_CAINFO}")
fi
